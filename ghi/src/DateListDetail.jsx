    ...
    return (
        <div className="container" style={{display: "flex", justifyContent: "center"}}>
            {allDateLists?.map((dateList) => {
                ...
                return (
                    <div key={dateList.id} id={dateList.id} className="card" style={{width: "25rem", height: "25rem", padding:"2rem", margin:"1rem", alignItems:"center"}}>
                        <h1>{changeDateFormat(dateList.date)}</h1>
                        {(dateList.description !== "") ?
                            (<div><Link to={`/packinglist/${packingListID}/datelists/${dateList.id}`} state={dateList}>{dateList.description}</Link></div>)
                        :
                            (<div><Link to={`/packinglist/${packingListID}/datelists/${dateList.id}`} state={dateList}>Add Description</Link></div>)
                        }
                        <div>{weatherCard}</div>
                        <div>{items}</div>
                        <div><input onChange={(e) => dispatch(handleNameChange(e.target.value))}></input></div>
                        <div><button value={dateList.id} onClick={handleSubmit}>Add Item</button></div>
                    </div>
                )
            })}
            <div className="card" style={{width: "25rem", height: "25rem", padding:"2rem", margin:"1rem", alignItems:"center"}}>
                <h1>Other Items</h1>
                    {misc_items?.map(item => {
                        return (
                            <div key={item.id}>
                ...
                <div><input onChange={(e) => dispatch(handleNameChange(e.target.value))}></input></div>
                <div><button value={null} onClick={handleSubmit}>Add Item</button></div>
            </div>
        </div>
    )
}
