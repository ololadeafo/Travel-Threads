...
@router.get(
    "/api/location/{country_id}/states", response_model=List[StatesOut]
)
def get_states_by_country(country_id: int, repo: LocationQueries = Depends()):
    states = repo.get_states(country_id)
    return states


@router.get(
    "/api/location/{province_type}/{province_id}/cities",
    response_model=List[CitiesOut],
)
def get_cities(
    province_type: str,
...
def get_city_info(city_id: int, repo: LocationQueries = Depends()):
    city = repo.get_one_city(city_id)
    return city


@router.get("/api/location/city/details/{city_id}")
def get_all_city_info(city_id: int, repo: LocationQueries = Depends()):
    city = repo.get_all_city_info(city_id)
    return city


@router.get("/api/location/state/details/{state_id}")
def get_all_state_info(state_id: int, repo: LocationQueries = Depends()):
    state = repo.get_all_state_info(state_id)
    return state


@router.get("/api/location/country/details/{country_id}")
def get_all_country_info(country_id: int, repo: LocationQueries = Depends()):
